...
This data is not linearly separable. We need a [nonlinear transformation](https://isopink.github.io/Linear-Model-L/). Let us take $x_1$ squared and $x_2$ squared. Then we get this: 
...
This raises a question: what if the dimensionality of the $Z$ space is too high? There's no need to worry. What we get is simply a longer $z$ vector, and we still only deal with inner product. Moreover, we still pass an $N \times N$ coefficient matrix to the QP solver, just like before. Nothing is chagned, so far. 
