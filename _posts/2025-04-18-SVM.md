...
Suppose the dimensionality of $mathcal{Z}$ is a million. Highlighted four points are pre-images of support vectors. They actually live in $mathcal{Z}$ space, and the margin too. Here is the remarkable part: Even in a million-dimensional space, the decision boundary can be defined using just four support vectors. And now, we come to the generalization result: 
