...
We want this to be true for every $n$ from $1$ to $N$. Let us go for the solution. The left term of equation above can be desribed using matrix notation as below: 
...
For now, let us discuss RBF for classfication. Since $+1$ or $-1$ is also real-value, we use RBF to classification. Our hypothesis will be formalized as below: 
...
As we done in [Lecture 2], we want to minimize squared error: 
...
The Nearest Neighbor (NN) method classifies a new data point by copying the label of its closest neighbor in the training set. While this is simple and intuitive, it creates abrupt decision boundaries and is sensitive to noise. This situation is illustarted below: 

![solution](/assets/images/rbf_4.svg) 

Radial Basis Function (RBF) models build on this idea by considering the influence of all data points, not just the closest one. Using smooth functions like the Gaussian, RBF assigns more weight to nearby points and less to distant ones, resulting in much smoother and more flexible boundaries.

Both methods rely on similarity, but RBF offers a more refined and robust way to capture patterns in the data.
