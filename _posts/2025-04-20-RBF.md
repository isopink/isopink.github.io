...
We’ve now seen how RBF can be used as a model on its own, and also how it serves as a kernel in support vector machines SVM. While they may look similar on the surface, their underlying mechanics are quite different—yet they often lead to surprisingly comparable results.

In the case of SVM, the RBF kernel is used to define the similarity between data points, and the final hypothesis is built using only a small number of support vectors. The model takes the form:
...
Here, each $\mathbf{x}_n$ is a support vector selected through optimization, and the coefficients $\alpha_n y_n$ are obtained by solving a quadratic programming problem that maximizes the margin. Since $\alpha_n$ is always non-negative, the sign of each term is determined by the label $y_n$. The number of terms is equal to the number of support vectors.

In contrast, a “straight” RBF model chooses its centers $\mu_k$ using unsupervised learning methods like K-means. The weights $w_k$ are learned via pseudo-inverse, and the final output is given by:
...
The two models differ not only in how the centers are selected but also in how the weights are learned. The RBF model uses fixed centers and simple linear regression (with optional classification via sign), while the SVM model integrates the label information deeply into its structure.

Interestingly, despite these differences, both models can produce similar decision boundaries—though not always equally well. In the example shown, both models use nine centers to ensure a fair comparison. However, the SVM has a natural advantage because its support vectors are chosen to optimally separate the classes, while the RBF centers come from unsupervised clustering, which may not align well with the decision boundary.

In short, while the models follow completely different learning paths—one through supervised margin maximization and the other through unsupervised clustering and regression—they can converge to functionally similar results. The key difference lies in how intelligently the model leverages label information.
