...
A key difference is that the centers $\mu_k$ are fixed ahead of time, usually via K-means, so only the weights $w_k$ need to be learned. Since the model is linear in $w$, we can compute them efficiently using the pseudo-inverse. Optionally, a bias term $b$ can be included.

##### 3.1. EM algorithm for $\gamma$

Another important parameter is $\gamma$, which controls the spread of the Gaussian. Its value greatly affects performance. Too small or too large $\gamma$ leads to poor results. A practical solution is to use an alternating optimization strategy—first fixing $\gamma$ to solve for $w_k$, then fixing $w_k$ to update $\gamma$ via gradient descent. This resembles the Expectation-Maximization (EM) algorithm. For more flexibility, each center can even have its own $\gamma_k$.

##### 3.2. RBF versus its SVM kernel 

So far, we’ve looked at RBF as a standalone model. But it’s also used in SVMs, where the RBF kernel defines similarity between data points. Despite different training methods, both approaches can yield similar results.
...
Although the full derivation is quite complex and not covered in detail in the lecture, it was shown that applying this form of regularization leads exactly to the RBF solution.
